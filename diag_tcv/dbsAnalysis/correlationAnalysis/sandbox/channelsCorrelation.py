...
def get_raw_signals(shot, isweep, ifreq_list, verbose=False):
    '''
    WRAPPER FUNCTION
    ...
    if nbpts_per_freq_ref==nbpts_per_freq_hop:
        ...
        rawSigDic = get_raw_signals_same_nbpts(shot=shot, isweep=isweep, ifreq_list=ifreq_list, verbose=verbose)
    
    else:
        if verbose:
            print(' === ref & hop have different nbpts per freq === ')
        rawSigDic = get_raw_signals_diff_nbpts(shot=shot, isweep=isweep, ifreq_list=ifreq_list, verbose=verbose)

    return rawSigDic


def get_raw_signals_diff_nbpts(shot, isweep, ifreq_list, verbose=False):
    '''
    Get the raw DBS signals (t, I, Q) for both channels
...
def get_raw_signals_same_nbpts(shot, isweep, ifreq_list, verbose=False):
    '''
    Get the raw DBS signals (t, I, Q) for both channels

...
'''
CAREFUL: 
you access the data with ifreq_list
typically if you have scanned 40 frequencies in the shot you can access them separately 
by calling ifreq from 1 to 40 and you can access the concatenated array with ifreq=0

BUT the frequencies themselves are labelled from 0 to 39 this is why, you have data_ref.params.F[ifreq-1]
in get_raw_signals
'''



def get_raw_signals(shot, isweep, ifreq_list, rawSigDic=None, numDemod=False, verbose=False):
    '''
    WRAPPER FUNCTION
    ...
    if nbpts_per_freq_ref==nbpts_per_freq_hop:
        ...
        rawSigDic = get_raw_signals_same_nbpts(shot=shot, isweep=isweep, ifreq_list=ifreq_list,rawSigDic=rawSigDic, numDemod=numDemod, verbose=verbose)
    
    else:
        if verbose:
            print(' === ref & hop have different nbpts per freq === ')
        rawSigDic = get_raw_signals_diff_nbpts(shot=shot, isweep=isweep, ifreq_list=ifreq_list,rawSigDic=rawSigDic, numDemod=numDemod, verbose=verbose)

    return rawSigDic


def get_raw_signals_diff_nbpts(shot, isweep, ifreq_list, rawSigDic=None, numDemod=False, verbose=False):
    '''
    Get the raw DBS signals (t, I, Q) for both channels
    ...
    for i, ifreq_loc in enumerate(ifreq_list):
        ...
        _t_ref, _I_ref, _Q_ref = data_ref.get_signal(ifreq_ref+1) #loading ifreq_ref
        _t_hop, _I_hop, _Q_hop = data_hop.get_signal(ifreq=ifreq_loc+1) #loading ifreq_hop
    ...
    rawSigDic['data_ref'] = data_ref
    rawSigDic['data_hop'] = data_hop
...
def get_raw_signals_same_nbpts(shot, isweep, ifreq_list, rawSigDic=None, numDemod=False, verbose=False):
    '''
    Get the raw DBS signals (t, I, Q) for both channels
    ...
    for i, ifreq_loc in enumerate(ifreq_list):
        ifreq_loc = int(ifreq_loc)
        _t_ref, _I_ref, _Q_ref = data_ref.get_signal(ifreq_loc+1)
        _t_hop, _I_hop, _Q_hop = data_hop.get_signal(ifreq_loc+1)

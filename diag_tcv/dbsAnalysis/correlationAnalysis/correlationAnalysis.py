...
class CorrelationAnalysis(TCVShot):
    ...
    def __init__(self, shot, numDemod=False, machine='tcv', verbose=False):
    ...
    def _load_sweep_params(self):
        if self.numDemod:
            self.dbs_ref = DataInterface(self.shot, isweep=False, channelval=3, machine='tcv')
            self.dbs_hop = DataInterface(self.shot, isweep=False, channelval=4, machine='tcv')
        else:
    ...
    def get_raytracing_data(self):
        ...
        if self.numDemod:
            output, beam3d_interface = _DBSbeam('tcv',shot=self.shot, isweep=self.isweep, xmode=0, channelval=4, verbose=True, plot=True)
        else:

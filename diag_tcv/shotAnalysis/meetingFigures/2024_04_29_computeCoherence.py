...
# %%
fig, ax = plot_1d([],[], grid=True)
ax.plot(-correlObject.rho_list_ref[0:19]+correlObject.rho_list_hop[1:20], corr_list, marker='+', color='teal', label='Pearson')
plt.plot(-correlObject.rho_list_ref[0:19]+correlObject.rho_list_hop[1:20], spectral_coh_list, marker='x', color='indigo', label='spectral')
ax.legend()
...
ax.text(np.mean(correlObject.rho_list_ref[0:19]), -.05, 'ref', color='darkslategrey', transform=ax.get_xaxis_transform(),
            ha='center', va='top')
